...
class Linear(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bias is not None:
            ...
            x = x + self.bias.to(x.dtype) # for use of JVP
        return x
...
class Conv2d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if b is not None:
            ...
            x = x + b.reshape(1, -1, 1, 1) # for use of JVP
        return x
...
#----------------------------------------------------------------------------
# Attention Module version using nn.Module instead of autograd.Function
...
class AttentionModule(torch.nn.Module):
    ...
    def forward(self, q, k):
        # Compute attention weights using FP32 for numerical stability
        with torch.amp.autocast(device_type='cuda', enabled=False):
            q_fp32 = q.float()
            k_fp32 = k.float() / np.sqrt(k.shape[1])
            
            # Compute attention weights
            w = torch.einsum('ncq,nck->nqk', q_fp32, k_fp32)
...
class UNetBlock(torch.nn.Module):
    ...
    def forward(self, x, emb):
        ...
        skip = self.skip(orig) if self.skip is not None else orig
        x = x + skip
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        # Mapping.
        self.map_noise_t = PositionalEmbedding(num_channels=noise_channels, endpoint=True) if embedding_type == 'positional' else FourierEmbedding(num_channels=noise_channels) # sigma emb for t
        self.map_noise_h = PositionalEmbedding(num_channels=noise_channels, endpoint=True) if embedding_type == 'positional' else FourierEmbedding(num_channels=noise_channels) # sigma emb for t-r
...
#----------------------------------------------------------------------------
@persistence.persistent_class
class MFPrecond(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        label_dim       = 0,                # Number of class labels, 0 = unconditional.
    ...
    def forward(self, x, t, class_labels=None, h=None, force_fp32=False, **model_kwargs):
        ...
        h = h + (1e-6 if self.use_fp16 else 1e-8) # to avoid log(0)
        t = t + (1e-6 if self.use_fp16 else 1e-8)
...
#         from easy_meanflow.training.networks_amf import SongUNet as AFMSongUNet
#         self.model = AFMSongUNet(
...
class AlphaWeightNet(torch.nn.Module):
    ...
    def __init__(self, img_channels, hidden_channels=128):
        super().__init__()
        self.net = torch.nn.Sequential(
            # Process alpha_r and alpha_t jointly
            torch.nn.Conv2d(img_channels * 2, hidden_channels, 3, padding=1),
            torch.nn.SiLU(),
...
class AMFPrecond(torch.nn.Module):
    def __init__(self,
        ...
        **model_kwargs,
        ...
        if use_weight:
            self.weight_net = AlphaWeightNet(img_channels)
    ...
    def forward(self, x, alpha_t, alpha_r, return_weight=False, **kwargs):
        dtype = torch.float16 if (self.use_fp16 and x.device.type == 'cuda') else torch.float32
