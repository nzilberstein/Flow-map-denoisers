...
class ImageTransformerDenoiserModelV2(nn.Module):
    def __init__(self, levels, mapping, in_channels, out_channels, patch_size, num_classes=0, mapping_cond_dim=0):
        ...
        self.time_in_proj_r = Linear(mapping.width, mapping.width, bias=False)
    ...
    def forward(self, x, sigma, sigma_r=None, aug_cond=None, class_cond=None, mapping_cond=None):
        # Patching
        x = x.movedim(-3, -1)
        x = self.patch_in(x)
        ...
        if sigma_r is not None:
            c_noise_r = torch.log(sigma_r.clamp(min=1e-6)) / 4
            time_emb_r = self.time_in_proj_r(self.time_emb_r(c_noise_r[..., None]))
        else:
            time_emb_r = 0
        ...
        cond = self.mapping(time_emb + time_emb_r + aug_emb + class_emb + mapping_emb)
